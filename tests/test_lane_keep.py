...
def test_cross_track_is_vs_s_centerline_not_world_y0():
    # First-lobe post sits near y = -1. World y=0 there is ~1 m off the path.
    ct = track_cross_track_m(7.5, -1.0)
    assert abs(ct) < 0.05
    ct_world_zero = track_cross_track_m(7.5, 0.0)
    assert ct_world_zero > 0.8


def test_nadir_deadband_holds_straight_at_32_29():
    steer, err = steer_from_nadir_gaps(32, 29, cruise=5.5)
    assert err == 0.0
    assert steer == 0.0
...
def test_ahead_hold_yaws_before_axle_moves():
    straight, _ = steer_from_nadir_gaps(32, 29, left_ahead_px=32, right_ahead_px=26, cruise=2.5)
    walking, _ = steer_from_nadir_gaps(32, 29, left_ahead_px=32, right_ahead_px=31, cruise=2.5)
    assert straight == 0.0
    assert walking is not None and walking > 0.0


def test_v_scale_raises_steer_at_champ_cruise():
    slow, _ = steer_from_nadir_gaps(24, 29, cruise=2.5)
    fast, _ = steer_from_nadir_gaps(24, 29, cruise=5.5)
...
def test_lane_keep_command_fights_32_29():
