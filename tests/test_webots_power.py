...
def test_build_webots_telemetry_payload():
    ...
    assert payload["locomotion"]["gait"] == "walk"


def test_stress_multiplier_increases_walk_transit_draw():
    base = {"Legs": 10.0, "Arms": 5.0, "Torso": 3.0, "Compute": 9.0}
    stressed = aggregate_channel_draws(
        {"left_wheel": 10.0},
        gait="walk",
        phase="walk_transit",
    )
    assert stress_multiplier(gait="walk", phase="walk_transit") > 2.0
